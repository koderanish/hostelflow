...
const docSchema = z.object({
  ...
  type: z.enum(['Aadhaar', 'PAN', 'Passport', 'Admission Letter', 'Fee Receipt', 'Medical Certificate', 'Other']),
...
export function UploadDocumentPage() {
  ...
  const { register, handleSubmit, setValue, formState: { errors } } = useForm<DocFormData>({
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader title="Upload Document" description="Upload a student document" />
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="space-y-1.5">
              ...
              <input type="text" {...register('fileName')} placeholder="e.g. aadhaar_rahul.pdf" className={inputClass} />
              {errors.fileName && <p className={errorClass}>{errors.fileName.message}</p>}
            </div>
            <div className="space-y-1.5">
              <label className={labelClass}>File URL</label>
              <input type="text" {...register('fileUrl')} placeholder="/files/document.pdf" className={inputClass} />
            </div>
          </div>
