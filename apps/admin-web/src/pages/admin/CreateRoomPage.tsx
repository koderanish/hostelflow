...
import type { Hostel, Building } from '../../types';
...
export function CreateRoomPage() {
  ...
  useEffect(() => {
    if (hostelId && !filteredBuildings.find(b => b.id === buildingId)) {
      setBuildingId('');
    }
  }, [hostelId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!hostelId || !buildingId || !roomNo || !price) {
    ...
    const payload = {
      ...
      roomNo,
      floor: Number(floor),
      roomType: roomType as 'Single' | 'Double' | 'Triple' | 'Dormitory',
