...
export function ReturnInventoryPage() {
  ...
  const handleReturn = async () => {
    ...
    setSubmitting(true);
    const res = await inventoryService.returnItem(item.id, qty, returnedBy.trim());
    if (res.success) {
      addToast(`Returned ${qty} ${item.unit}(s) successfully`, 'success');
      navigate(`/admin/inventory/${item.id}`);
    } else {
      setError(res.error || 'Failed to return item');
    }
    setSubmitting(false);
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader
        ...
        description={`Return ${item.name} to inventory`}
        actions={
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6">
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between py-2 border-b border-slate-100 dark:border-slate-800">
              ...
              <span className="text-sm font-bold text-blue-600">{item.availableQuantity} {item.unit}</span>
            </div>
            <div className="flex items-center justify-between py-2 border-b border-slate-100 dark:border-slate-800">
              <span className="text-xs text-slate-500">Currently Issued</span>
              <span className="text-sm font-medium text-blue-600">{item.quantity - item.availableQuantity} {item.unit}</span>
            </div>
          </div>
        </div>

        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6">
          <h3 className="text-sm font-semibold text-slate-900 dark:text-white mb-4">Return Details</h3>
          <div className="space-y-4">
            <div className="space-y-1.5">
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Quantity to Return *</label>
              <input type="number" value={quantity} onChange={e => { setQuantity(e.target.value); setError(''); }}
                min={1} max={item.quantity - item.availableQuantity} className={inputClass} />
            </div>
            <div className="space-y-1.5">
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Returned By *</label>
              <input type="text" value={returnedBy} onChange={e => { setReturnedBy(e.target.value); setError(''); }}
                placeholder="e.g. Student name or staff" className={inputClass} />
            </div>
            ...
            <button onClick={handleReturn} disabled={submitting}
              className="w-full flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-blue-600 hover:from-blue-500 hover:to-indigo-600 text-white text-sm font-medium transition-all disabled:opacity-50 disabled:cursor-not-allowed">
