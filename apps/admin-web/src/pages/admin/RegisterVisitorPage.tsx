...
export function RegisterVisitorPage() {
  ...
  const { register, handleSubmit, setValue, formState: { errors } } = useForm<VisitorFormData>({
  ...
  const onSubmit = async (data: VisitorFormData) => {
    ...
    setSubmitting(true);
    const res = await visitorService.registerVisitor({
      visitorName: data.visitorName,
      phone: data.phone,
      idProofType: data.idProofType || undefined,
      idProofNo: data.idProofNo || undefined,
    ...
    if (res.success) {
      addToast('Visitor registered successfully', 'success');
      navigate('/admin/visitors');
    } else {
      addToast((res as any).error || 'Failed to register visitor', 'error');
    }
    setSubmitting(false);
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader title="Register Visitor" description="Register a new visitor entry" />
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div className="space-y-1.5">
              <label className={labelClass}>Visitor Name *</label>
              <input type="text" {...register('visitorName')} placeholder="e.g. Rajesh Kumar" className={inputClass} />
              {errors.visitorName && <p className={errorClass}>{errors.visitorName.message}</p>}
            </div>
            <div className="space-y-1.5">
              <label className={labelClass}>Phone *</label>
              <input type="text" {...register('phone')} placeholder="e.g. 9876543210" className={inputClass} />
              {errors.phone && <p className={errorClass}>{errors.phone.message}</p>}
            </div>
            <div className="space-y-1.5">
              <label className={labelClass}>ID Proof Type</label>
              <select {...register('idProofType')} className={inputClass}>
            ...
            <div className="space-y-1.5">
              ...
              <input type="text" {...register('idProofNo')} placeholder="e.g. 1234-5678-9012" className={inputClass} />
            </div>
            <div className="space-y-1.5">
              <label className={labelClass}>Relation *</label>
              <input type="text" {...register('relation')} placeholder="e.g. Father, Mother, Brother" className={inputClass} />
              {errors.relation && <p className={errorClass}>{errors.relation.message}</p>}
            </div>
            <div className="space-y-1.5">
            ...
            <div className="space-y-1.5">
              ...
              <input type="text" {...register('purpose')} placeholder="e.g. Family visit" className={inputClass} />
            </div>
          </div>
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-200 dark:border-slate-800">
            <button type="button" onClick={() => navigate('/admin/visitors')}
